/**
 * ISO 4217 circulating currencies.
 * Symbols resolve via Intl where available.
 */
...
export const CURRENCY_CODES = WORLD_CURRENCIES.map((c) => c.code) as [
  CurrencyCode,
  ...CurrencyCode[],
];

const currencySet = new Set<string>(CURRENCY_CODES);

export function isCurrencyCode(value: string): value is CurrencyCode {
  return currencySet.has(value);
}

/** Popular currencies shown first in selects. */
...
export function getCurrenciesForSelect() {
  const priority = new Set(PRIORITY_CODES);
  const rest = WORLD_CURRENCIES.filter((c) => !priority.has(c.code));
  return [
    ...PRIORITY_CODES.map(
      (code) => WORLD_CURRENCIES.find((c) => c.code === code)!,
    ),
    ...rest,
  ];
}
